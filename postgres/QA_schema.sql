...
DROP TABLE IF EXISTS questions, answers, answer_photos;
...
CREATE INDEX mult_col_idx_questions ON questions(product_id, question_helpfulness DESC);

CREATE INDEX mult_col_idx_answers ON answers(question_id, helpfulness DESC);

CREATE INDEX idx_photos ON answer_photos(answer_id);
...
SELECT SETVAL(pg_get_serial_sequence('answer_photos', 'id'), (SELECT max(id) FROM answer_photos));